...
class OllamaClient:
    ...
    def __init__(self, api_url=None, model_name=None, timeout=None, max_retries=None, retry_delay=None):
        """
    ...
    def generate(self, prompt: str, options: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"Error generating from model: {response.status_code}, {response.text}")
                raise requests.HTTPError(f"Error generating from model: {response.status_code}, {response.text}")
